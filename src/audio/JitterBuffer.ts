...
export class JitterBuffer {
  ...
  private readonly BYTES_PER_TICK = 160; // 160 samples @ 8kHz × 1 byte (A-Law/PCMA for Telnyx)
  ...
  /**
   * Generate comfort noise at -60dBFS (A-Law encoded for Telnyx PCMA)
  ...
  private generateCng(): Buffer {
    ...
    for (let i = 0; i < this.BYTES_PER_TICK; i++) {
      buf[i] = (Math.round((Math.random() * 2 - 1) * this.CNG_AMPLITUDE) ^ 0x55) & 0xFF;
    }
    return buf;
  }
}
